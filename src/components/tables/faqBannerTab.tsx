...
export default function FaqBannerEditor() {
  ...
  // Fetch the existing FAQ banner
  const getFaqBanner = async () => {
    setLoading(true);
    setError("");
    try {
      const res = await axios.get(
        `${import.meta.env.VITE_APP_URL}api/admin/faq-banner`,
        {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (!file && !banner) {
        setError("Please upload a FAQ Banner image.");
        setSaving(false);
        return;
      }

      const form = new FormData();
      if (file) form.append("banner", file); // column name is 'banner'
      ...
      setSuccess("FAQ Banner updated successfully.");
    } catch (err) {
      setError(
        err.response?.data?.message ||
          err.message ||
          "Failed to save FAQ Banner"
      );
    } finally {
      setSaving(false);
    }
        ...
        <>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex items-center gap-3">
              ...
              >
                {banner ? "Update FAQ Banner" : "Upload FAQ Banner"}
              </button>
            </div>
          </form>
        </>
